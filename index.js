...
app.post('/api/generate-bookmarklet', (req, res) => {
    ...
    const bookmarkletPayload = `javascript:(function(){let _t=null;const _r=/eyJ[a-zA-Z0-9_-]+\\.[a-zA-Z0-9_-]+\\.[a-zA-Z0-9_-]+/g;try{for(let _v of Object.values(localStorage)){if(typeof _v==='string'){let _m=_v.match(_r);if(_m){for(let _k of _m){try{let b=_k.split('.')[1].replace(/-/g,'+').replace(/_/g,'/');while(b.length%4)b+='=';let p=JSON.parse(atob(b));if(p.sub||p.user_id){_t=_k;break;}}catch(e){}}if(_t)break;}}}}catch(err){}if(!_t){let _m=document.cookie.match(/token=([^;]+)/);if(_m){let _cm=_m[1].match(_r);if(_cm)_t=_cm[0];}}if(!_t){let _l=localStorage.getItem('token');if(_l){let _lm=_l.match(_r);if(_lm)_t=_lm[0];}}if(_t){window['fetch']('${botUrl}/api/save-pixai-token',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({token:_t,nonce:'${nonce}'})}).then(r=>r.json()).then(d=>{window.alert(d.message);}).catch(e=>{window.prompt('Koneksi otomatis gagal. Salin Kode ini lalu paste di Opsi B Web Auth:',_t+'|${nonce}');});}else{window.alert('Token tidak ditemukan. Pastikan Anda sudah login di pixai.art');}})()`;
