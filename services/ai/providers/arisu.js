 ...
 */
async function generate({ prompt, senderId, isOwner, model, systemPrompt = null, useMemory = true, throwOnError = true }) {
    ...
    try {
        ...
        if (response.data.success && response.data.data && response.data.data.message) {
            ...
            throw new Error(`Arisu ${modelEndpoint} gagal: ${typeof apiMessage === 'string' ? apiMessage : JSON.stringify(apiMessage)}`);
        }
    } catch (error) {
