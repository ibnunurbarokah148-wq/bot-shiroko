...
async function handleChat(bot, username, message, mcData) {
    ...
    if (pk.startsWith('aimode') || pk.startsWith('!aimode')) {
        ...
        if (!targetMode) {
            const currentMode = state.mcAiMode || (globalState.userAiMode ? (globalState.userAiMode[ownerId] || (Array.isArray(ID_OWNER) && globalState.userAiMode[ID_OWNER[1]])) : null) || 'copilotku';
        ...
        const validModes = ['gemini', 'ollama', 'openrouter', 'or', 'cloudflare', 'cf', 'copilotku', 'ds3', 'ds4', 'glm', 'qwen', 'arisu-gemini', 'gpt', 'grok'];
    ...
    try {
        ...
        // Mode AI ditentukan dari: 1. Mode in-game jika ada -> 2. Mode WA Owner -> 3. Fallback Copilotku
        const ownerId = Array.isArray(ID_OWNER) ? ID_OWNER[0] : ID_OWNER;
        const activeAiMode = state.mcAiMode || (globalState.userAiMode ? (globalState.userAiMode[ownerId] || (Array.isArray(ID_OWNER) && globalState.userAiMode[ID_OWNER[1]])) : null) || 'copilotku';
